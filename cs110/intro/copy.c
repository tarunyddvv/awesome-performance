...
int main(int argc, char *argv[]) {
  ...
  while (true) {
    ...
    while (bytesWritten < bytesRead) {
      ssize_t result = write(fdout, buffer + bytesWritten /* this is a pointer inside the buffer where the reamining data starts */,
                                   bytesRead - bytesWritten /* remaining bytes to write; how much remaining data exists */);
      if (result == -1) {
        perror("write");
        close(fdin);
        close(fdout);
